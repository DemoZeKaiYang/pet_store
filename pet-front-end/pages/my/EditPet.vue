  ...
  <uni-card>
    <uni-forms label-align="left" :modelValue="formData" ref="form">
      <uni-forms-item class="pet-picture">
        <uni-file-picker limit="1" v-model="imageValue" fileMediatype="image" :auto-upload="false" @select="select"
      ...
      <uni-forms-item label="品种:" name="pet_kind_id">
        <uni-data-select v-model="formData.pet_kind_id" :localdata="kinds" @change="change"></uni-data-select>
      </uni-forms-item>
      <uni-forms-item label="绝育:" name="pet_sterilize">
        <uni-data-select v-model="formData.pet_sterilize" :localdata="sterilize">
        </uni-data-select>
      </uni-forms-item>
      ...
      <uni-forms-item label="出生:" name="pet_birthday">
        <uni-datetime-picker type="date" :clear-icon="false" v-model="formData.pet_birthday" 
          ref="date" :end="end" />
      </uni-forms-item>
    </uni-forms>
...
<script setup>
  ...
  const imageValue = computed(() => {
    if (!formData.value.pet_avatar && formData.value.pet_avatar === "") return {}
    return {
