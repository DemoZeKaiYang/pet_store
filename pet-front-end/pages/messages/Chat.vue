<template>
	<view class="">
		<scroll-view scroll-y="true" class="scroll-box" :style="{'height': (windowObj.windowHeight - windowObj.statusBarHeight - 94 + 'px')}" :scroll-top="scrollHeight" @scrolltoupper="loadMores">
			<view class="scroll-view">
				<view class="news-box" v-for="(item,index) in list" :key="index">
					<image class="avatar" :class="[item.isMe? 'is-me': 'avatar-right']" :src="item.avatar" mode="aspectFill"></image>
					<view class="message-box" :class="{'is-me': item.isMe}">
						<text class="message" v-if="item.type !== 2">{{item.content || ''}}</text>
						
						<image class="message-image" :src="item.content" mode="aspectFill" v-else @tap="tapTo(3)"></image>
					</view>
				</view>
			</view>
		</scroll-view>
	...
	export default {
		...
		methods: {
			tapTo(state) {
				switch(state) {
					case 1:
						this.image = 'https://nimg.ws.126.net/?url=http%3A%2F%2Fdingyue.ws.126.net%2F2021%2F0826%2F6def4faej00qygdfw009kd200u001hcg00u001hc.jpg&thumbnail=660x2147483647&quality=80&type=jpg'; // 上传的图片路径
						this.newsSend(2)
						break
					case 2:
						this.newsSend(1)
			...
			newsSend(type) {
				...
				uni.sendSocketMessage({
					...
					complete: (res) => {
						...
						message["type"] = type;
						message["avatar"] = 'https://nimg.ws.126.net/?url=http%3A%2F%2Fdingyue.ws.126.net%2F2021%2F0826%2F6def4faej00qygdfw009kd200u001hcg00u001hc.jpg&thumbnail=660x2147483647&quality=80&type=jpg'; // 自己的头像
						delete message['data']
			...
			connectSocket() {
				let that = this;
				let param = that.data, userInfo = that.userInfo;
