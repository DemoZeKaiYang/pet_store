    ...
    <view class="category-list">
      ...
      <scroll-view scroll-y="true" class="left">
        ...
        <view
          ...
          :class="[kind.good_kind_order == showCategoryIndex ? 'on' : '']"
          @click="showCategory(kind.good_kind_order)"
        >
          <view class="text">
            <view class="block"></view>
            {{ kind.good_kind_name }}
        ...
        >
          ...
          <view>
            <view style="text-align: center">
              ...
              <text style="color: #000000; margin-right: 8px; margin-left: 8px">{{
                kind.good_kind_name
              }}</text>
              <text style="color: gainsboro">—</text>
            </view>
            <view class="list" v-if="kind.children.length > 0">
              <view
                class="box"
                v-for="(category, i1) in kind.children"
                :key="category.good_category_order"
                @click="toCategory(category.good_category_id)"
              >
                <image :src="`${devUrl}/good_uploads/` + category.good_category_image"> </image>
