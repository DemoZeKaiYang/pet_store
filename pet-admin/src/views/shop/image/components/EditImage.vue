  ...
  <el-dialog :model-value='dialogFormVisible' title='编辑商品' width='600' @close='cancelHandler'>
    <el-form :model='formData' class='kind-form' :rules='rules' ref='formRef' show-message>
      ...
      <el-form-item label='商品名称 :' size='large' prop='good_category_name'>
        <el-select
          v-model='formData.good.good_name'
          placeholder='请选择商品名称：'
          size='large'
...
import { uploadGoodImageAPI ,updateGoodImageAPI} from '@/apis/shop/good_image/index.js'
...
const confirm = async (formEl) => {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      const result = await updateGoodImageAPI(formData.value)
      if (result.code === 2000) {
        successMessage('修改成功')
        resetFormData()
        emit('cancelDialog')
