<template>
  <el-card class='top-edit'>
    <el-button type='primary' size='large' style='font-size: 20px' @click='addKind' disabled>添加地址</el-button>
    <el-button type='danger' size='large' style='font-size: 20px' @click='delSelectKind'>删除选中</el-button>
    <!-- 搜索框 -->
    <el-input
      v-model.trim='search'
      placeholder='请输入要搜素的收件人姓名'
      size='large'
      class='pet-search'
      prefix-icon='Search'
    />
  ...
  <el-card class='contain'>
    ...
    >
      ...
      <el-table-column label='用户姓名:' width='200' prop='user_name' />
      <el-table-column label='地址电话:' width='200' prop='address_phone' />
      <el-table-column label='收件姓名:' width='150' prop='address_name' />
      <el-table-column label='收件省市:' width='250' prop='address_area' />
      <el-table-column label='详细地址:' width='300' prop='address_details' />
      <el-table-column label='默认地址:' prop='address_default' width='150'>
        <template #default='scope'>
          <el-tag type='danger' v-if='scope.row.address_default===1'>默认地址</el-tag>
          <el-tag type='primary' v-else>普通地址</el-tag>
        </template>
      </el-table-column>
...
import EditGood from './components/EditAddress.vue'
...
import { delAddressAPI, getAddressAPI, searchAddressAPI } from '@/apis/address/index.js'
...
const getData = async () => {
  ...
  if (result.code === 2000) {
    tableData.value = result.data.map(item => {
      let obj = { ...item, user_name: item.user.user_name }
      delete obj['user']
      return obj
    })
  }
...
const delSelectKind = async () => {
  ...
  if (confirmDel) {
    const address_id_arr = selectData.value.map((item) => item.address_id)
    const result = await delAddressAPI(address_id_arr)
    if (result.code === 2000) {
      successMessage('删除成功')
      getData()
    } else {
...
const delKind = async (row) => {
  const confirmDel = await delMessageBox()
  if (confirmDel) {
    const result = await delAddressAPI(row.address_id)
    if (result.code === 2000) {
      successMessage('删除成功')
      getData()
    } else {
...
const searchBtn = async () => {
  ...
  const result = await delAddressAPI(search.value)
  if (result.code === 2000) {
    tableData.value = result.data.map(item => {
      let obj = { ...item, user_name: item.user.user_name }
      delete obj['user']
...
const handleEdit = (index, row) => {
  let obj = {
    ...row,
    address_area:row.address_area.split('-')
  }
  console.log(obj)
  editData.value = obj
