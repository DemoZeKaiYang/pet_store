    ...
    >
      ...
      <el-table-column label="编辑" width="300">
        <template #default="scope">
          ...
          <el-button size="large" type="info" @click="showAdopt(scope.row)">查看</el-button>
        </template>
      </el-table-column>
    </el-table>
  </el-card>

  <el-drawer v-model="drawer" title="对应的领养宠物" direction="ltr" :before-close="handleClose">
    <div class="good-content">
      <img :src="imagePrefix + showAdoptData.adopt_image" />
      <div class="text">
        <div class="name">{{ showAdoptData.adopt_name }}</div>
        <div class="number">出生日期 : {{ showAdoptData.adopt_birthday }}</div>
        <div class="total">性别 : {{ showAdoptData.adopt_sex }}</div>
        <div class="total">品种 : {{ showAdoptData.adopt_var }}</div>
        <div class="price">地区 : {{ showAdoptData.adopt_address }}</div>
        <div class="total">描述 : {{ showAdoptData.adopt_description }}</div>
        <div class="total">领养状态 : {{ showAdoptData.adopt_status === 1 ? '未领养' : '已领养' }}</div>
      </div>
    </div>
  </el-drawer>
</template>
